...
import { etfHoldingsLimitSchema, etfTickerSchema } from "../shared/schemas";

export function registerEtfHoldingsTool(server: FastMCP, api: LlmquantWebApiClient) {
  server.addTool({
    name: "etf_holdings",
    description:
      "Retrieve the latest available SEC N-PORT regulatory holdings for an ETF " +
      "in LLMQuant Data's allowlisted free-source universe. Holdings are sorted " +
      ...
      "Credit rule (approved 2026-05-12): full / partial / stale consume 1 " +
    ...
    execute: async ({ ticker, limit }) => {
      try {
        const response = await api.getEtfHoldings({ ticker, limit });
        const { coverage_status, ticker: normalized } = response.data;
        const summary =
          coverage_status === "unsupported"
            ? `${normalized} is not supported by the SEC-first ETF MVP — see coverage_notice.`
