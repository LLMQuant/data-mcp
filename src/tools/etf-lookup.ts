...
import { etfTickerSchema } from "../shared/schemas";

export function registerEtfLookupTool(server: FastMCP, api: LlmquantWebApiClient) {
  server.addTool({
    name: "etf_lookup",
    description:
      ...
      "summary, and source / coverage metadata from LLMQuant Data's allowlisted " +
      "free-source ETF universe.\n\n" +
      ...
      "the SEC-first MVP; use this tool for fund context, NOT for price bars — " +
    ...
    execute: async ({ ticker }) => {
      try {
        const response = await api.getEtfLookup({ ticker });
        ...
        const summary =
          coverage_status === "unsupported"
            ? `${label} is not supported by the SEC-first ETF MVP — see coverage_notice.`
            : `${label} latest SEC N-PORT snapshot as_of_date=${response.data.as_of_date ?? "unknown"} (${coverage_status}).`;
